...
func TestCreateRejectsSameKeyForDifferentRequest(t *testing.T) {
	repository := &memoryRepository{records: map[string]Record{
		"request-1": {
			ID: "TRK-existing", IdempotencyKey: "request-1", UserID: "user-1",
			Channel: ChannelJD, ExternalProductID: "sku-1", Source: "search",
		},
	}}
	service := NewService(repository, func() string { return "TRK-new" }, time.Now)
	_, err := service.Create(context.Background(), CreateInput{
		IdempotencyKey: "request-1", UserID: "user-2", Channel: ChannelJD,
		ExternalProductID: "sku-1", Source: "search",
	})
