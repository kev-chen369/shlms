...
type shareEventFunc func(context.Context, conversion.NativeShareEventInput) (conversion.ShareRecord, error)

func (f shareEventFunc) RecordShareEvent(ctx context.Context, in conversion.NativeShareEventInput) (conversion.ShareRecord, error) {
	return f(ctx, in)
}
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			d := Dependencies{Users: identityFunc(func(*http.Request) (string, error) { return "u1", tc.identityErr }),
				NativeShareEvents: shareEventFunc(func(_ context.Context, in conversion.NativeShareEventInput) (conversion.ShareRecord, error) {
					called = true
...
func TestShareEventsPostgresReplayConflictAndNoNewTracking(t *testing.T) {
	...
	userID := "u1"
	router := NewRouterWithDependencies(Dependencies{Users: identityFunc(func(*http.Request) (string, error) { return userID, nil }), NativeShareEvents: conversion.Repository{DB: db}})
