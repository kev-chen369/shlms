...
func TestShareEventConcurrentReplayConflictAndOwnerIsolation(t *testing.T) {
	...
	in := NativeShareEventInput{OwnerUserID: "u1", LinkID: r.ID, EventID: "event1", Action: "copy_link", Scene: "group"}
	const workers = 12
	...
	for _, change := range []func(*NativeShareEventInput){
		func(in *NativeShareEventInput) { in.Action = "copy_text" },
		func(in *NativeShareEventInput) { in.Scene = "social" },
	} {
		changed := in
		change(&changed)
...
func TestShareEventRejectsInvalidUnauthorizedAndPendingWithoutWrites(t *testing.T) {
	...
	in := NativeShareEventInput{OwnerUserID: "u1", LinkID: r.ID, EventID: "event1", Action: "copy_text", Scene: "group"}
	if _, err := repo.RecordShareEvent(context.Background(), in); !errors.Is(err, ErrStateConflict) {
		t.Fatal(err)
	}
	for _, tc := range []struct {
		change func(*NativeShareEventInput)
		want   error
	}{
		{func(in *NativeShareEventInput) { in.OwnerUserID = "u2" }, ErrNotFound},
		{func(in *NativeShareEventInput) { in.LinkID = "missing" }, ErrNotFound},
		{func(in *NativeShareEventInput) { in.EventID = "" }, ErrInvalid},
		{func(in *NativeShareEventInput) { in.Action = "delivered" }, ErrInvalid},
		{func(in *NativeShareEventInput) { in.Scene = "bad\x00scene" }, ErrInvalid},
	} {
		changed := in
		tc.change(&changed)
...
func TestShareEventMigrationRoundTripAndOwnershipConstraint(t *testing.T) {
	...
	if _, err := repo.RecordShareEvent(context.Background(), NativeShareEventInput{OwnerUserID: "u1", LinkID: r.ID, EventID: "event1", Action: "copy_link", Scene: "group"}); err != nil {
		t.Fatal(err)
	}
}
