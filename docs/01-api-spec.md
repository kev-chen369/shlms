...
京东渠道位配置增量实现：`PUT /admin/v1/promotion-positions/{id}/channels/JD` 要求独立管理员认证和 `channel:position:configure` 权限，JSON 正文为 `{accountId, externalPositionId, version}`，首次配置 version=0，后续更新带当前配置版本；必须提供 Idempotency-Key。配置只会进入 PENDING_VERIFICATION，客户端不能上传 readiness、READY 或所属用户。响应仅对受权管理员返回映射详情。目标不存在 404、版本或外部位重复绑定 409、推广身份未启用 403、渠道不支持 422、内部错误 503。不能凭该接口返回结果开放转链；真实渠道核验和生产装配另行完成。
