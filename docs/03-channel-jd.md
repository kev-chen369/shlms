...
2026-09-17 用户确认拟使用的 APP 对外名称为「万宝单生活」。这只是显示名称，不能替代 Android / iOS 包名、应用下载地址、实际投放来源、京东审核后的 APP ID（`siteId`）及 AppKey/AppSecret。控制台目前没有可改名的应用；待用户提供真实包名或网站域名并完成京东登记后，再记录获批媒体 ID，不预填示例值。
...
`cmd/jd-promotion` 是仅供运营 / 联调人员在受控环境中手动验证一个商品的命令，复用 `jd.Adapter` 与签名 `HTTPClient`；它不是用户端 API，也不会写 Tracking、订单、佣金或渠道 READY 状态。部署环境通过密钥管理注入 `JD_APP_KEY`、`JD_APP_SECRET`，通过已核验的备案信息配置 `JD_SITE_ID`，确认已获批场景 2 后才设置 `JD_SCENE2_APPROVED=true`。未获批时保持关闭。可选 `JD_SUBUNION_APPROVED=true` 仅在另行核验该权限后使用；默认不发送 `subUnionId`。

满足上述条件并核对实际商品与外部推广位后，在 Go 1.26 环境执行：

```text
go run ./cmd/jd-promotion -product 23484023378 -position 6
```

`-product` 支持京东商品编号或严格的 `https://item.jd.com/{sku}.html`；`-position` 填京东外部推广位的正整数 ID，不是本项目内部位 ID。只有经授权且需要按内部 Tracking 归因时，再传 `-tracking`，同时预先确认子标识权限。不要在命令参数、文档或 CI 日志中写 AppSecret、登录密码或授权 Key。成功时仅在命令标准输出打印经过 HTTPS 域名检查的推广链接；失败输出通用错误且不自动重试。测试账号未创建合规媒体 / 应用，因此现在没有可执行的真实成功命令。
...
- 命令测试：缺凭据 / 站点、未获批子标识、非法商品与非数值推广位均无输出、无真实网络调用；与其余 Go / PostgreSQL 集成测试一起运行。
