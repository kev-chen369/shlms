...
图中金额、订单、链接均为演示数据，不作为生产默认值。现有图覆盖首页与 16 个核心页面，并已补审核中、拒绝、停用、无推广位四屏；渠道未配置、价格变动、过期、其他空数据、失败重试仍需补图。审核后台和团队页尚未绘制。交付前完成对应状态稿和交互验收。
...
执行器进展（M2-03c-2b）：内部执行器已定义获批渠道 Gateway 的创建、按稳定请求号查询和链接域名校验契约；执行前再次核验预览报价和推广位，CAS 领取后最多调用一次创建。结果不确定只进入查询恢复，查询 UNKNOWN 时不重发。`cmd/api` 未装配执行器，当前没有真实渠道客户端或自动任务调度；模拟 Gateway 的测试仅证明流程防重，不能证明京东接口支持该请求号或结果查询。正式接入前必须验证渠道能力，并在预留事务中再次核验 READY。
