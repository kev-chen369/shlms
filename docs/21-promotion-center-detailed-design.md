...
受限 HTTPS 展开器已落地：禁用代理，逐跳重验域名和 DNS，先验证地址再直接拨号所选公网 IP，TLS 仍按原始渠道主机校验证书；设置跳转、总时长、响应头与响应体上限。此组件仅返回安全取得的最终链接和受限内容，商品 ID、价格、优惠券及个人收益须由已授权渠道接口核验后才能形成预览；尚未开放用户端预览请求。
