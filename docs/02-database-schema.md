...
`000013_coupon_claims` 保存消费者领券请求的用户归属、券、幂等键、请求指纹和 `PENDING / QUERY_REQUIRED / CLAIMED / FAILED` 状态。每人同键唯一；只有 `CLAIMED` 可附脱敏回执引用，终态不能被较晚失败覆盖。当前仅存储边界，生产尚无领取适配器或用户可调用的领券接口；该表记录不等于平台账户已收到券。
