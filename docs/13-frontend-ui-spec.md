...
领券页实现进展（M6-04a）：平台切换后从同源 API 分页读取已核验券，显示优惠额、门槛、范围、有效期，并可查看详情及适用商品。空列表、未开放平台、请求失败和失效券均有明确状态。页面只有「查看规则」，领取、去使用和平台跳转尚未开放。页面读数及操作能力仍需真实渠道物料和终端验收。

城市筛选进展（M6-04b-1）：`GET /api/v1/coupon-cities?platform=...` 从当前有效且已核验、带城市名称的券物料提取选项；页面默认「不限城市活动」，选城市后同时展示该城市与不限城市券。平台或城市切换会清掉旧详情与分页游标，列表、详情、适用商品及首页券预览使用同一地域上下文。无获批物料时城市选项为空，不人工显示示例城市。
