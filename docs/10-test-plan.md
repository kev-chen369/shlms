...
## M7-01e-3c-1 实际选品浏览器增量（2026-09-19）

运行中的本机 H5（默认127.0.0.1:5173）及既有 Playwright / Chrome，通过 `npm run test:materials` 重复执行6项实际组件 / 页面测试。三个宽度320 / 390 / 1280px覆盖默认与200% CSS文本、长合法位 / 场景 / 物料字段、44px点击目标、原生输入高度、无横向溢出 / 框架覆盖、键盘本人位选择 / 显式范围确认、推广位与目录续页503保留及重试、空目录 / SUSPENDED、详情503重试 / 新鲜复核 / EXPIRED移除、商品 / 活动与平台切换。200% CSS文字不是浏览器缩放或系统字体验收。

竞态用例延迟真实 uni GET 的 HTTP 响应，不替换生产模型：旧列表 / 详情在平台切换后不得恢复；旧身份401不得拒绝新身份；同身份在视图变化后的401仍清理私有位；身份C→D→C使旧C响应失效、不自动重读；未失焦原生城市草稿不得带入新身份。所有调用为GET，鉴权仅由测试端合成身份提供，错误响应私有消息不显示。真实身份 / 来源 / 渠道、微信真机和视觉图归档仍分别验收；该增量不勾选父项。
