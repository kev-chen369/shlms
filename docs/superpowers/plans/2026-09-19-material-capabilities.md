...
## M7-01e-3c-1 实际选品浏览器验收（已完成，2026-09-19）

新增 `tests/browser/materials-acceptance.browser.cjs`，测试host加载同一注册页面、生产模型和真实 uni GET；仅身份 Ref 与 HTTP 响应由测试端合成。`npm run test:materials` 聚合原组件 / 页面两项及新增三宽度状态 / 一项竞态，共6项。保留既有 `test:layout` 七项单独执行，不将历史数量冒充本轮新增覆盖。

- [x] 320 / 390 / 1280px 默认与200% CSS文字：长合法位 / scene / 物料、44px目标、原生输入高度、无横向溢出 / overlay；键盘显式选位 / 确认与最新范围，位和目录分页503保留 / 重试，目录empty / SUSPENDED，详情503重试 / 最新标题 / EXPIRED移除，活动开始时间、类型清草稿、未开放平台零请求及退出清私有数据。
- [x] 实际HTTP延迟列表与详情，切平台后不恢复；旧身份401不得影响新身份，同身份迟到401在类型改变后仍清位；身份C→D→C使旧响应失效且不自动读，原生未失焦草稿不跨身份。释放后等待指定响应体完成，再检查页面，不仅检查释放前状态；竞态20秒超时。
- [x] 最终6项浏览器、全量27文件485项单测、类型检查、H5 / 微信构建退出0；独立只读6项实跑审查无阻断，文档相对链接 / 语法 / diff通过。生产页面 / 模型未修改，不接管另一个任务的京东API。

七张截图保存在Git外，PNG签名 / 非零尺寸通过，已直接查看窄屏商品长标题、桌面事实与本人位放大；长内容允许纵向滚动而非缩小字号。完整逐张审阅 / 归档及V3差异说明由M7-01e-3c-2独立完成，3c / e / M7总体仍进行中。不是浏览器缩放、系统字体、辅助技术或微信设备验收；真实身份、渠道来源 / 权限与业务另验收。本轮逐项本地commit，不push / 部署。
...
空目录与capability拒绝区分为empty / blocked及safe reason；capability拒绝清整个目录和其他在途响应。分页可恢复错误保留有效卡片与游标，重复ID或同游标循环不追加，禁止并发分页。详情每次新读；失效卡片移除，有效卡片更新摘要，两者均废止旧续页 / cursor并提示刷新，避免旧页复活失效数据。没有生成 / 预览状态，不构造演示物料，不用客户端时钟提升授权。

- [x] 缺模块RED后初版24项模型测试GREEN（真实Vue / API适配函数，仅HTTP边界合成）；测试表terminal类型推宽后改用Partial<MaterialScope>[]保留合法枚举。
- [x] 独立初版24项及diff实跑无阻断，按Minor建议补详情503 / 404保持目录且可重试、旧身份401不影响已加载新身份，并补session ABA旧成功响应隔离回归；无生产改动，复核既有行为。
- [x] 最终新增28项、全量23文件417项、typecheck、H5 / 微信构建退出0，15计划链接 / diff通过，独立只读初版24项竞态审查无阻断；仅补测试后最终全量再跑通过，逐项提交。e-3页面 / 浏览器 / 实际图尚未开始，不以模型完成勾选父项。

## M7-01e-1 前端列表 / 详情读取契约（已完成，2026-09-19）

在当前功能分支实现，无其他tracked改动，保留.DS_Store和其他工作树。新增materials-api.ts，GET仅发送平台 / 类型 / 终端 / 内部位 / 场景及可选地域 / 业务，固定20项分页；不接受owner / media / READY输入。保存请求范围快照，可信身份由既有session提供、不生成测试身份；身份改变不接收在途结果。尾页nextCursor可省略归一为空串，详情只接受规范非零UUID且必须与响应一致。

公开卡片显式白名单，平台类型和地域 / 业务 / 终端必须匹配，活动有startsAt且不虚构商品价，UTF8字节限制、RFC3339与纳秒时间窗口校验，数组独立拷贝。READY仅为CATALOG.allowed，能力拒绝不带items / cursor，详情能力 / availability / item保持一致；未知和冲突状态安全错误，不借客户端时钟推断授权。普通JavaScript平台映射继承属性曾绕过白名单，新增3项实际RED（错误响应被接受），自有属性检查修复；独立审查也实见该边界，不依赖后端400掩盖前端错误。

- [x] 缺模块RED、初版43项GREEN；测试桩mock调用参数类型修正，补详情 / 拷贝 / 纳秒 / envelope及继承属性回归。
- [x] 独立增量审查指出hasOwnProperty会隐式转换数组或String对象；两项实际RED后加严格平台字符串检查，防止错误类型变成有效枚举。
- [x] 最终全量客户端测试22文件389项 / typecheck / H5与微信构建退出0，15文档链接 / diff通过；独立最终51项定向实跑无阻断，修复两个平台白名单缺口后独立提交。e-2状态模型、e-3页面和浏览器验收尚未开始；无视觉改动、无新图片、无生产发布或自动push。

## M7-01d-3b-2 部署文件与真实启动入口（已完成，2026-09-19）

在现有功能分支实施，开始时无其他tracked改动；保留.DS_Store和其他工作树。可选CATALOG_BINDINGS_FILE为空即关闭，显式配置必须绝对路径普通文件；Lstat拒绝叶子符号链接 / FIFO / 目录，非阻塞打开及SameFile复核防替换为非普通对象，最多读取64KiB+1再严格解析，统一错误无路径 / 内容。父目录由可信运维控制；一次加载复制、无热重载，不替代逐请求数据库授权。

- [x] 文件 / 配置组装缺接口RED后实现GREEN；成功、未设置 / []关闭、缺文件 / 目录 / FIFO / symlink / 相对路径 / 非法 / 超限配置安全拒绝。
- [x] 实际main错误配置子进程失败，未监听且无配置路径 / 测试值泄漏；有效文件→临时API二进制→真实RSA签名 / 隔离PG→HTTP列表 / 详情，空设置默认关闭，编辑文件不重载，数据库SUSPENDED新请求拒绝，零Tracking / 转链请求。
- [x] 临时将main的CatalogBindings丢弃，真实入口验收RED为UNCONFIGURED；恢复接线后重新验证，不靠源码grep证明装配。
- [x] 定向三轮race / 最终全量Go、vet / build / 文档链接 / diff与独立审查通过，逐项提交。综合复核d-1～3后更新父项；不补勾选品前端、来源真实性、正式身份 / 权限 / 设备或渠道生成。

最终私有PG_TEST_DSN三包定向-race -count3及全量go test -count=1 ./...通过；首次vet拒绝新增测试跨包无字段名结构体字面量，修为显式字段名后重新全量test / vet / build退出0。独立只读审查实跑加载 / 真实main失败 / 实际二进制HTTP通过，无Critical / Important。测试结束非系统schema数0，无残留业务写入；图稿无变化，本地提交、不推送或部署。父项仅完成已约定只读后端范围，不缩小M7-01总体目标，e前端和f真实来源、其他生成 / 订单 / 资金及发布门槛仍待完成。
