...
### PROC-17 选品页面增量同步（2026-09-19）

同步三个已完成任务：M7-01e-3a 本人推广位安全读取（273f676）、M7-01e-3b-1 只读选品组件（673c841）、M7-01e-3b-2 完整选品页面与推广入口（5d836d6）。用户需明确选择本人推广位并确认服务端场景，才读取目录；支持城市 / 业务筛选、商品 / 活动详情及失效边界。身份、平台或类型变化清理旧范围。淘宝 / 美团没有支持的本人映射时显示未开放；目录读取不授权生成，生成链接仍禁用。未改动另一个任务负责的京东适配器。

本轮重新运行 `npm test`：27 文件、485 项通过；`npm run typecheck`、`npm run build:h5`、`npm run build:mp-weixin` 退出 0。现有 `npm run test:layout` 7 项及 `node --test tests/browser/materials-component.browser.cjs tests/browser/materials-page.browser.cjs` 2 项通过。Browser 插件不可用，使用现有 Playwright / Chrome，在 `http://127.0.0.1:5173` 验证入口、实际页面、显式选位 / 确认、平台与身份清理、键盘及无非预期控制台错误。选品页面基础流程为 390px；完整多宽度、文本放大与迟到请求验收 M7-01e-3c 仍进行中，其未提交测试及计划改动不纳入本次发布。

新增[实际选品页面图](./images/client-materials-selection-390-20260919.png)，本轮重新生成并直接查看：本人位选择、固定场景、城市 / 业务确认、目录未配置和返回推广中心。只在测试端提供合成身份及 HTTP 响应，不是生产账号或真实渠道验收。继续保留 V2 / V3 概念图，不用概念商品图片、价格或收益伪装已实现页面。

发布采用干净工作树合并已提交历史、非强制推送并独立核对远端；结果见[PROC-17 计划](./superpowers/plans/2026-09-19-github-material-page-sync.md)。生产部署入口尚未提供，更新 GitHub 不代表线上 H5 / 微信版本已发布；不执行生产迁移、不连接真实渠道。
