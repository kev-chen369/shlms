...
推广员侧新增 P-01～P-16 功能映射，详见 [推广中心模型与用户端设计](./20-promoter-model-user-ui-design.md)。

推广模块正式详细设计及首页 P-00 已纳入[推广中心详细设计](./21-promotion-center-detailed-design.md)，全部页面逐项关联路由、接口、图稿和 M1～M5 批次；对应可勾选任务见[研发计划](./superpowers/plans/2026-09-16-promotion-center.md)。

2026-09-16 基线更新：以下早期进度描述保留作历史记录；Tracking PostgreSQL 迁移、仓储及并发测试已有代码，生产启动依赖装配和真实京东调用仍未完成。推广中心新页面与业务接口尚待开发，不应由 F-06 基础能力推断为已完成。
